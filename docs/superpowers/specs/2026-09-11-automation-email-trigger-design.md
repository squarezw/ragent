...
> **勘误（2026-09-14 补充）**：上文第 1 条低估了"够不着"的程度。向导的**触发方式选择器里没有「邮件触发」这一项**——它被 `2fbf927`（tozawali，2026-09-08，"fix: hide email trigger and fix automation run persistence"，该提交正文未记录原因）连同向导内的整套邮箱配置 UI 一并删除。因此本方案实施期间，step 2 的邮件区块只能经由**模板**（「客户询价处理」「售后投诉处理」两个模板写死 `trigger: "邮件触发"`）或**编辑既有的邮件触发自动化**进入，从空状态直接创建是进不去的。该选择器项已在本分支恢复（工程实现见 §十二）；`2fbf927` 删除的内联邮箱表单则由模块 B 重建。
...
> **勘误与前置条件（2026-09-14 补充，实施完成后回写）**
>
> 1. **第 1 条以「邮件触发」可从触发方式选择器直接选中为前提**，而该前提在本次实施期间并不成立（见 §一 勘误）。选择器项已在本分支恢复；在那之前，这条只能经模板或编辑既有自动化满足。**上线前请确认恢复选择器这一改动确实进入发布**，否则整个模块 B 的界面用户无从到达。
> 2. **第 6 条按字面无法满足，规格自相矛盾**：它要求"不存在任何 system 分支"，而 §四 模块 A 的 A.4 又**要求**存在一个识别 `"system"` 并拒绝的分支（`RETIRED_SYSTEM_MAILBOX_KEY`）。实现按后者执行——识别该值并**抛错**，绝不路由进已下线行为。建议把本条改写为：「不存在任何**路由进已下线行为**的 system 分支或兜底；识别并拒绝该哨兵值的校验分支除外」。
> 3. **末条按字面无法满足**：`pnpm check:ci` 在**未改动的 `main` 上就已失败**（工作区是 CRLF 检出而 biome 配置为 LF，约 1750 条既有诊断）。任何分支都无法满足它。实际采用的判据是「**不新增诊断**」（逐 (file, rule) 计数对比）。建议以该表述替换本条。
> 4. **`pnpm test` 有一条先存在的失败**（`test/appsTenantFilter.test.ts`「租户列表拉取失败不影响整页」），在本分支开始前的 `main` 上即失败，与本次改动无关。判据应为「除该条外全部通过，且该条仍以同样原因失败」。
