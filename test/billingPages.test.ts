...
test("系数页把在吃默认值的条目摆出来，且撤销的语义不含糊", () => {
  assert.ok(RATES.includes("usingDefault"), "没有展示在吃默认值的条目");
  // 撤掉一个显式系数 = 回落到全局默认，不是变免费。两者算出的钱可能一样，
  // 含义相反。这里断言语义而不是某句固定措辞 —— 文案可以改，
  // 「说成免费」不行。
  assert.match(RATES, /toast\.success\("[^"]*默认[^"]*"\)/,
    "撤销后的提示没说清是回落默认");
  assert.doesNotMatch(RATES, /toast\.success\("[^"]*免费[^"]*"\)/,
    "撤销提示里出现了「免费」——那是另一回事（显式设 0）");
});
